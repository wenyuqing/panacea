...
class QueryRoIHeadv3(BaseModule):
    ...
    def __init__(self,
        ...
        super(QueryRoIHeadv3, self).__init__(init_cfg)
        ...
        self.geo_embed = nn.Sequential(
                nn.Linear(self.geo_channels, conv_out_channels),
                nn.ReLU(),
        ...
        if self.num_branch_convs > 0:
            for i in range(self.num_branch_convs):
                ...
                if conv_in_channels != conv_out_channels:
                    branch_convs.append(
                    ConvModule(
                        conv_in_channels,
                        conv_out_channels,
                    ...
                    branch_convs.append(BasicBlock(conv_in_channels, conv_out_channels))
                ...
                #         conv_cfg=self.conv_cfg,
                #         norm_cfg=self.norm_cfg))
            last_layer_dim = conv_out_channels * 2
        ...
        self.context_embed = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(last_layer_dim, self.embed_dims),
            )
    ...
    def forward(self, data, img_metas, eps=1e-5):
        ...
        context_embedding = self.context_embed(geo_feats)
        ...
        for img_id, (bboxes, labels) in enumerate(data["proposal"]):
            ...
            # print(coords3d[..., 2:3].min(), coords3d[..., 2:3].max())
            mask = (coords3d[..., 2] > -5 ) * (coords3d[..., 2] < 3)
            coords3d = coords3d[mask, :]
            # print(mask.sum() / coords3d.size(0))
        ...
        context_embedding_list = [context_embedding[:batch_reference_point_list[0].size(1)], context_embedding[batch_reference_point_list[0].size(1):]]
        print(context_embedding.size(), batch_reference_point_list[0].size(), batch_reference_point_list[1].size())
        max_size = 600
        for img_id, reference_point in  enumerate(batch_reference_point_list):
            if reference_point.size(1) < max_size:
                ...
                padded_reference_point = torch.rand_like(padded_reference_point)
                reference_point = torch.cat([reference_point, padded_reference_point], 1)
                batch_reference_point_list[img_id] = reference_point

                context_embed = context_embedding_list[img_id]
                print(context_embed.size())
                padded_context_embed = context_embed.new_full((max_size - context_embed.size(0), 256), 0)
                context_embed = torch.cat([context_embed, padded_context_embed], 0).unsqueeze(0)
                print(context_embed.size())
                context_embedding_list[img_id] = context_embed

        reference_points = torch.cat(batch_reference_point_list, 0)
        context_embedding = torch.cat(context_embedding_list, 0)
        print(context_embedding.size())
        # print(reference_points.min(), reference_points.max())
        return reference_points


@HEADS.register_module()
class JointHeadv3(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is JointHeadv3):
        ...
        super(JointHeadv3, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
