...
class QueryRoIHead(BaseModule):
    ...
    def __init__(self,
        ...
        last_layer_dim = self.embed_dims 
        ...
        if self.num_branch_convs > 0:
            ...
            last_layer_dim = conv_out_channels + self.embed_dims 
        ...
        self.reference_point_layer = nn.Linear(fc_out_channels // 2, 3)
    ...
    def forward(self, data, img_metas, eps=1e-5):
        ...
        intrinsic = torch.stack([data['intrinsics'][..., 0, 0] , data['intrinsics'][..., 1, 1], data['intrinsics'][..., 0, 2], data['intrinsics'][..., 1, 2]], dim=-1) / 1e3
        ...
        # img_feats = [img_feat.flatten(0, 1).detach() for img_feat in data['img_feats']] if self.detach else [img_feat.flatten(0, 1).detach() for img_feat in data['img_feats']]
        bbox_feats = self.bbox_roi_extractor(img_feats, rois).float()
        if self.num_branch_convs > 0:
            for conv in self.branch_convs:
                bbox_feats= conv(bbox_feats)
        bbox_feats = self.avg_pool(bbox_feats)
        bbox_feats = bbox_feats.flatten(1)

        coords_d = torch.ones_like(centers[..., 0:1])
        ...
        coord_rv = coords_rv.unsqueeze(-1)
        img2lidars = data['lidar2img'].inverse().flatten(0, 1)
        coords3d_point_list = []
        for img_id, (bboxes, labels) in enumerate(data["proposal"]):
            img2lidar = img2lidars[img_id].unsqueeze(0)
            coord_rv = coords_rv[rois[:, 0] == img_id, :].unsqueeze(-1)
            coords3d = torch.matmul(img2lidar, coord_rv).squeeze(-1)[..., :3]
            coords3d_point_list.append(coords3d)
        coords3d = torch.cat(coords3d_point_list, 0)
        geo_feats = self.geo_embed(pos2posemb3d(coords3d))
        geo_feats = torch.cat([bbox_feats, geo_feats], -1)

        if self.num_branch_fcs > 0:
            for fc in self.branch_fcs:
                geo_feats = self.relu(fc(geo_feats))
        
        batch_feats_list = [geo_feats[rois[:, 0] < N, :].unsqueeze(0), geo_feats[rois[:, 0] > (N -1), :].unsqueeze(0)]
        max_size = max(batch_feats_list[0].size(1), batch_feats_list[1].size(1))
        for img_id, single_feat in enumerate(batch_feats_list):
            if single_feat.size(1) < max_size:
                padded_single_feat = single_feat.new_full((1, max_size - single_feat.size(1), single_feat.size(-1)), 0)
                single_feat = torch.cat([single_feat, padded_single_feat], 1)
                batch_feats_list[img_id] = single_feat
        instance_feats = torch.cat(batch_feats_list, 0)

        if self.num_branch_fcs > 0:
            for fc in self.branch_fcs:
                geo_feats = self.relu(fc(geo_feats))
        
        context_embedding, position_embedding = torch.split(instance_feats, self.embed_dims, dim=-1)
        reference_points = self.reference_point_layer(position_embedding)
        reference_points = reference_points.sigmoid()

        return context_embedding, reference_points
...
class JointHead(AnchorFreeHead):
    ...
    def forward(self, img_metas, **data):
        ...
        if self.query_head is None:
            ...
            context_embedding, reference_points = self.query_head(data, img_metas)
        ...
        #     tgt = torch.zeros_like(query_pos)
        # else:
        #     padded_embedding = context_embedding.new_full((B, query_pos.size(1) - context_embedding.size(1), context_embedding.size(-1)), 0)
        #     tgt = torch.cat([padded_embedding, context_embedding], 1)
