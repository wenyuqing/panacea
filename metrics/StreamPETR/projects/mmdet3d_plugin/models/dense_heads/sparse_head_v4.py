...
class SparseHeadv4(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is SparseHeadv4):
        ...
        super(SparseHeadv4, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def init_weights(self):
        ...
        if self.using_queries or not self.using_context:
    ...
    def forward(self, img_metas, 
        ...
        if self.using_queries:
            ...
            # proposal_positions = proposal_positions.detach()
            reference_points = (proposal_positions[..., :3] - self.pc_range[:3]) / (self.pc_range[3:6] - self.pc_range[0:3])
            # print(reference_points[..., :2].min(), reference_points[..., :2].max(), reference_points[..., 2:].min(), reference_points[..., 2:].max())
            # reference_points = torch.clamp(reference_points, min=0, max=1)
            if not self.using_context:
                init_reference_points = self.reference_points.weight
                init_reference_points = init_reference_points.unsqueeze(0).repeat(B, 1, 1)
                reference_points = torch.cat([reference_points, init_reference_points[:,:self.num_query-reference_points.size(1),:]], 1)
            else:
        ...
        if self.using_queries or not self.using_context:
            ...
            tgt = self.context_embedding(pos2posemb3d(reference_points))
            tgt[:, :query_embeds.size(1), :] = query_embeds
