...
class Petr3DWindow(MVXTwoStageDetector):
    """Petr3DWindow."""
    ...
    def __init__(self,
                 ...
                 single_test=False,
                 pretrained=None):
        super(Petr3DWindow, self).__init__(pts_voxel_layer, pts_voxel_encoder,
    ...
    def forward_sliding_test(self, img_metas, rescale, **data):
        ...
        for i in range(T):
            ...
            cur_img_metas = [img_meta[i] for img_meta in prev_img_metas]
            location = self.prepare_location(cur_img_metas, **data_t)
            outs_roi = self.forward_roi_head(location, **data_t)
            topk_indexes = outs_roi['topk_indexes']

            if data_t['prev_exists'] == False:
                self.pts_bbox_head.reset_memory()
            _ = self.pts_bbox_head(location, cur_img_metas, topk_indexes, **data_t)
        ...
        cur_img_metas = [img_meta[-1] for img_meta in img_metas]
        location = self.prepare_location(cur_img_metas, **rec_data)
        outs_roi = self.forward_roi_head(location, **rec_data)
        topk_indexes = outs_roi['topk_indexes']

        outs = self.pts_bbox_head(location, cur_img_metas, topk_indexes, **rec_data)

        out_bbox_list = self.pts_bbox_head.get_bboxes(outs, cur_img_metas)
        bbox_results = [
