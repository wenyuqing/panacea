...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def init_from_ckpt(
        ...
        if path.endswith("ckpt") or path.endswith("ckpt'"):
            sd = torch.load(path, map_location="cpu")["state_dict"]
        elif path.endswith("safetensors"):
            sd = load_safetensors(path)
        else:
        ...
        sd_vae = {}
        for name in sd:
            if 'first_stage_model' in name and 'conditioner' not in name:
                sd_vae[name.replace('first_stage_model.','')] = sd[name]
        if len(sd_vae) == 0 :
            sd_vae = sd
        missing, unexpected = self.load_state_dict(sd_vae, strict=False)
        print(
            f"Restored from {path} with {len(missing)} missing and {len(unexpected)} unexpected keys"
        )
        if len(missing) > 0:
    ...
    def log_images(self, batch: Dict, **kwargs) -> Dict:
        ...
        if len(x.shape) == 5: # train vae, video original input
            log["inputs"] = rearrange(x, "b t c h w -> (b t) c h w")   
        else:
            log["inputs"] = x
