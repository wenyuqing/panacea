        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    if hasattr(embedder, "input_key") and (
                            embedder.input_key == "final_cond_zero" or embedder.input_key == "cond_img"
                    ):
                        # if embedder.input_key == "cond_bev":
                        #     cond = batch['cond_img']
                        # else:
                        cond=batch[embedder.input_key]
                        cond = rearrange(cond, "b t c h w -> (b t) c h w ").contiguous()
                        emb_out = embedder(cond)
                    else:
...
class IdentityEncoder(AbstractEmbModel):
    ...
    def forward(self, x):
        return x
class DownSampler(AbstractEmbModel):
    def __init__(self, dims=2,model_input_channels=4):
        super().__init__()
        self.input_hint_block = TimestepEmbedSequential(
            conv_nd(dims, 13, 16, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, 16, 16, 3, padding=1, stride=2),
            nn.SiLU(),
            conv_nd(dims, 16, 32, 3, padding=1, stride=2),
            nn.SiLU(),
            conv_nd(dims, 32, 32, 3, padding=1, stride=2),
            nn.SiLU(),
            zero_module(conv_nd(dims, 32, model_input_channels, 3, padding=1))
        )
    def forward(self,x):
        x = self.input_hint_block(x,None)
        return x
...
class FrozenOpenCLIPEmbedder2(AbstractEmbModel):
    ...
    def __init__(
        ...
        ## if the network is not good, you can download it offline and load the downloaded checkpoints
        # model, _, _ = open_clip.create_model_and_transforms(
        #     arch,
        #     device=torch.device("cpu"),
        #     pretrained="/data/code/ckpts/CLIP-ViT-bigG-14-laion2B-39B-b160k/open_clip_pytorch_model.bin",
        #     cache_dir="./ckpts"
        # )
        del model.visual
        self.model = model
...
class FrozenOpenCLIPEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        assert layer in self.LAYERS
        model, _, _ = open_clip.create_model_and_transforms(
            arch, device=torch.device("cpu"), pretrained=version,cache_dir="./ckpts"
        )
        ## if the network is not good, you can download it offline and load the downloaded checkpoints
        # model, _, _ = open_clip.create_model_and_transforms(
        #  arch, device=torch.device("cpu"), pretrained="/data/code/ckpts/models--laion--CLIP-ViT-H-14-laion2B-s32B-b79K/snapshots/94a64189c3535c1cb44acfcccd7b0908c1c8eb23/open_clip_pytorch_model.bin", cache_dir="./ckpts"
        # )
        del model.visual
        self.model = model
