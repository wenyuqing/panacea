...
from ...modules.attention import SpatialTransformer,SpatialTemporalTransformer
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb):
        ...
        #         return checkpoint_new_deepspeed(
    ...
    def _forward(self, x, emb):
        if self.updown:  ## x 512 32 28 28 emb 512 128
...
class ResBlock3D(TimestepBlock):
    ...
    def __init__(
        ...
        self.use_scale_shift_norm = use_scale_shift_norm
        self.exchange_temb_dims = exchange_temb_dims
        self.alpha= 1 if alpha == 1 else nn.Parameter(th.rand(1, requires_grad=True))
        ...
        self.in_layers_temporal = nn.Sequential(
            normalization(self.out_channels),
            nn.SiLU(),
            zero_module(conv_nd(1, self.out_channels, self.out_channels, self.temporal_kernel_size, padding=self.temporal_kernel_size // 2)),
        )
    ...
    def _forward(self, x, emb):
        if self.updown:  ## x 512 32 28 28 emb 512 128 ####TO DO yuqing 230801
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
            x = self.x_upd(x)
            ...
            h = self.in_layers(x)
            ## add temporal
            identity=self.identity_layer(h)
            BT, C, H, W = h.shape
            h = rearrange(h, "(b t) c h w -> (b h w) c t ", t=self.num_frames).contiguous() ## TO DO,t changeable
            h = self.in_layers_temporal(h)
            h = rearrange(h, "(b h w) c t -> (b t) c h w", h=H,w=W).contiguous()
            #h = self.alpha * identity + (1 - self.alpha) * h
            h = identity + h
        ...
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None]
        if self.use_scale_shift_norm: ####TO DO yuqing 230801
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]
            scale, shift = th.chunk(emb_out, 2, dim=1)
            h = out_norm(h) * (1 + scale) + shift
            h = out_rest(h)
        else:
            if self.exchange_temb_dims: ####TO DO yuqing 230801
                emb_out = rearrange(emb_out, "b t c ... -> b c t ...")
            h = h + emb_out
            h = self.out_layers(h)
            ## add temporal
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        #     if x.grad is not None and x.grad_fn is not None:
        #         return checkpoint_new_deepspeed(
        #             self._forward, (x, ), self.use_checkpoint
        #         )
        #     else:
        return checkpoint(
...
class AttentionBlock3D(nn.Module):
    ...
    def _forward(self, x):
        ...
        x = rearrange(x, "(b t) c h w -> (b h w) c t", t=self.num_frames).contiguous()
        qkv = self.qkv(self.norm_temporal(x))
        h = self.attention_temporal(qkv)
        h = self.proj_out_temporal(h)
        out = x + h
        out = rearrange(out,"(b h w) c t -> (b t) c h w" ,h=spatial[0],w=spatial[1]).contiguous()
...
class UNetModel3D(nn.Module):
    ...
    def __init__(
                        ...
                        layers.append(
                            ...
                            if not use_spatial_transformer
                                ...
                                SpatialTemporalTransformer(
                                    ...
                                    insert_crossview=insert_crossview,
                                )
                            )
                        )
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            if not use_spatial_transformer
                ...
                SpatialTemporalTransformer(  # always uses a self-attn
                    ch,
                    num_heads,
                    dim_head,
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        # expand context
        #context = context.half() # to save mem, not quite sure
