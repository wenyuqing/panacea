...
class Petr3DP(MVXTwoStageDetector):
    """Petr3DP."""
    ...
    def __init__(self,
                 ...
                 single_test=False,
                 pretrained=None):
        super(Petr3DP, self).__init__(pts_voxel_layer, pts_voxel_encoder,
    ...
    def obtain_history_memory(self,
        ...
        for i in range(T):
            ...
            loss = self.forward_pts_train(gt_bboxes_3d[i],
                                        gt_labels_3d[i], gt_bboxes[i], gt_labels[i], img_metas[i], 
                                        centers2d[i], depths[i], gt_future_traj[i], gt_future_traj_mask[i], 
    ...
    def forward_pts_train(self,
        ...
        if return_losses:
            loss_inputs = [gt_bboxes_3d, gt_labels_3d, gt_future_traj, gt_future_traj_mask, outs]
    ...
    def forward(self, return_loss=True, **data):
        ...
        """
        if return_loss:
            for key in ['gt_future_traj', 'gt_future_traj_mask','gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'img_metas']:
    ...
    def forward_train(self,
        ...
        losses = self.obtain_history_memory(gt_bboxes_3d,
                        ...
                        gt_labels, img_metas, centers2d, depths,  gt_future_traj, gt_future_traj_mask, gt_bboxes_ignore, **data)
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        outs = self.pts_bbox_head(location, img_metas, topk_indexes, **data)
        bbox_list, traj_list = self.pts_bbox_head.get_bboxes(
            outs, img_metas)
        bbox_results = [
            bbox3d2result(bboxes, scores, labels)
            for bboxes, scores, labels in bbox_list
        ]
        bbox_results[0]['traj'] = traj_list[0][0]
        bbox_results[0]['traj_scores'] = traj_list[0][1]
        return bbox_results
