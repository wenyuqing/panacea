...
class PETRMotionHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRMotionHead):
        ...
        super(PETRMotionHead, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        self.traj_embedding = nn.Embedding(self.mode, self.embed_dims)
        traj_branch = []
        for _ in range(self.num_reg_fcs):
            traj_branch.append(Linear(self.embed_dims, self.embed_dims))
            traj_branch.append(nn.ReLU())
        traj_branch.append(Linear(self.embed_dims, self.pred_steps*2 + 1))
        traj_branch = nn.Sequential(*traj_branch)
        self.traj_branches = nn.ModuleList(
            [traj_branch for _ in range(self.num_pred)])
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRMotionHead:
            convert_dict = {
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            outputs_traj = self.traj_branches[lvl](outs_dec[lvl].unsqueeze(-2) +  self.traj_embedding.weight)
        ...
        all_traj_tmp = torch.stack(outputs_trajs)

        all_bbox_preds[..., 0:3] = (all_bbox_preds[..., 0:3] * (self.pc_range[3:6] - self.pc_range[0:3]) + self.pc_range[0:3])
        # num_layer, bs. num_query, mode, step*5
        all_traj_scores = all_traj_tmp[..., 0]
        all_traj_preds = all_traj_tmp[...,1:].unfold(-1, 2, 2).contiguous()
        # print(all_traj_preds[-1, 0, 0])
        all_traj_preds[..., :2] = torch.cumsum(all_traj_preds[..., :2], dim=-2)
        all_traj_preds[..., :2] += all_bbox_preds[..., :2].unsqueeze(-2).unsqueeze(-2).detach()
        ...
        if mask_dict and mask_dict['pad_size'] > 0:
            ...
            outputs_traj = all_traj_preds[:, :, mask_dict['pad_size']:, :]
            outputs_traj_scores = all_traj_scores[:, :, mask_dict['pad_size']:, :]
            mask_dict['output_known_lbs_bboxes']=(output_known_class, output_known_coord)
            outs = {
                'all_cls_scores': outputs_class,
                'all_bbox_preds': outputs_coord,
                'dn_mask_dict':mask_dict,
                'all_traj_preds': outputs_traj,
    ...
    def _get_target_single(self,
        ...
        traj_targets = torch.zeros_like(traj_pred[..., :2])
        traj_weights = torch.zeros_like(traj_pred[..., :2])
        traj_weights_mask = torch.ones_like(traj_pred[..., :2])
        # DETR
        if sampling_result.num_gts > 0:
            bbox_targets[pos_inds] = sampling_result.pos_gt_bboxes
            bbox_weights[pos_inds] = 1.0
            traj_targets[pos_inds] = gt_trajs[sampling_result.pos_assigned_gt_inds].unsqueeze(1)
            traj_weights[pos_inds] = 1.0
            traj_weights_mask[pos_inds] = gt_trajs_mask[sampling_result.pos_assigned_gt_inds].unsqueeze(1)
            traj_weights = traj_weights * traj_weights_mask

            labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
        return (labels, label_weights, bbox_targets, bbox_weights, traj_targets, traj_weights,
                pos_inds, neg_inds)
    ...
    def get_targets(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, traj_targets_list, traj_weights_list, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_target_single, cls_scores_list, bbox_preds_list, traj_preds_list,
             gt_labels_list, gt_bboxes_list, gt_trajs_list, gt_trajs_mask_list, gt_bboxes_ignore_list)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_weights_list, bbox_targets_list, 
                bbox_weights_list, traj_targets_list, traj_weights_list, num_total_pos, num_total_neg)

    def loss_single(self,
                    cls_scores,
                    bbox_preds,
                    traj_probs,
                    traj_preds, # B, N, step, 2
                    gt_bboxes_list,
                    gt_labels_list,
                    gt_trajs_list, # B, M, step, 2
                    gt_trajs_mask_list, # B, M, step, 2
                    gt_bboxes_ignore_list=None):
        ...
        traj_preds_list = [traj_preds[i] for i in range(num_imgs)]

        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list, traj_preds_list,
                                           gt_bboxes_list, gt_labels_list, gt_trajs_list,
                                           gt_trajs_mask_list, gt_bboxes_ignore_list)
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list, 
        traj_targets_list, traj_weights_list, num_total_pos, num_total_neg) = cls_reg_targets
        ...
        loss_traj = self.loss_traj(traj_probs, traj_preds, traj_targets, traj_weights, avg_factor=num_total_pos)
        loss_cls = torch.nan_to_num(loss_cls)
        loss_bbox = torch.nan_to_num(loss_bbox)
        loss_traj = torch.nan_to_num(loss_traj)
        return loss_cls, loss_bbox, loss_traj
    ...
    def loss(self,
             gt_bboxes_list,
             gt_labels_list,
             gt_future_trajs_list,
             gt_future_trajs_mask_list,
             preds_dicts,
             gt_bboxes_ignore=None):
        """"Loss function.
        Args:
        ...
        all_traj_probs = preds_dicts['all_traj_scores']
        ...
        all_gt_trajs_list = [gt_future_trajs_list for _ in range(num_dec_layers)]
        all_gt_trajs_mask_list = [gt_future_trajs_mask_list for _ in range(num_dec_layers)]

        losses_cls, losses_bbox, losses_traj = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds, all_traj_probs, all_traj_preds,
            all_gt_bboxes_list, all_gt_labels_list, 
            all_gt_trajs_list, all_gt_trajs_mask_list,
            all_gt_bboxes_ignore_list)
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_traj_i in zip(losses_cls[:-1],
                                           losses_bbox[:-1], losses_traj[:-1]):
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            labels = preds['labels']
            traj = preds['traj'].cpu()
            traj_scores = preds['traj_scores'].cpu()
            ret_list.append([bboxes, scores, labels])
            traj_list.append([traj, traj_scores])
        return ret_list, traj_list
